...
public class TablePage extends JPanel{
	...
	private ActionListener prevBtnListener;
	private ActionListener nextBtnListener;
	public TablePage(ClientFrame mainFrame, ActionListener prevBtnListener, ActionListener nextBtnListener){
		this.mainFrame = mainFrame;
		doc = mainFrame.getDocument();
		this.prevBtnListener = prevBtnListener;
		this.nextBtnListener = nextBtnListener;
	...
	private String[][] fillPage(){
		...
		int index = (begIndex<=0)?0:(begIndex-1);
		for(int i=0+index;i<rowCount+index;i++){
			try{
				tempCell = noteToStringArr(notes.get(i));
			}catch(Exception ex){
				tempCell = null;
			}
			currentPageData[i-index] =  tempCell; 
		}
		return currentPageData;
	}
	
	private String[] noteToStringArr(Note note){
	...
	private void initListeners(){
		prevBtn.addActionListener(prevBtnListener);
	...
	public void setPageNumberEdit(int pageNumber){
		pageNumberEdit.setValue(pageNumber);
	}
