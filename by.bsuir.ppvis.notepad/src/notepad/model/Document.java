...
public class Document {
	...
	public int getMaxWidth(){
		...
		while(i<model.length()){
			if(valid){
				currentWidth+=getCharWidth(getCharAt(i));
			} else {
				if(maxWidth<currentWidth) maxWidth = currentWidth;
				currentWidth = 0;
				valid = true;
			}
			if(model.charAt(i)==KeyChar.NEWLINE){
				++lineCount;
				valid = false;
			}
			++i;
	...
	public int getMaxHeight(){
		int lineAmount = getLineCount();
		return lineAmount*getLineHeight();
	}
	...
	public void setBegSelectPos(int pos){
		if(pos<0){
			...
			begSelectPos = model.length()-1;
		} else {
			if(pos!=endSelectPos)begSelectPos = pos;
			else begSelectPos = -1;
		}
	}
	...
	public void setEndSelectPos(int pos){
		if(pos<0){
			...
			if(pos!=begSelectPos)endSelectPos = pos;
			else endSelectPos = -1;
		}
	}
